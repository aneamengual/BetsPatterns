...
import dataAccess.AnadirApuestaParameter;
   ...
   public Question createQuestion(Event event, String question, float betMinimum) throws EventFinished, QuestionAlreadyExist{
	...
	public Usuario anadirUsuario(String nombre, String apellido1, String apellido2, int telefono, String correo,
		...
		Usuario user=null;
		 ...
		 user=dbManager.anadirUsuario(nombre, apellido1, apellido2, telefono, correo, nombreUsuario, fechaNacimiento, DNI, password, numTarjeta);		
	...
	public Apuesta anadirApuesta(Usuario user, Pronostico respuesta, float apuesta, String modoPago, Event ev, int year){
		dbManager.open(false);		
		Apuesta a= dbManager.anadirApuesta(user, respuesta, new AnadirApuestaParameter(apuesta, modoPago, year), ev);
		dbManager.close();
		return a;
	}
	...
	public ArrayList<Usuario> getListaNegra(int year) {
		dbManager.open(false);
		ArrayList<Usuario> ln=dbManager.getListaNegra(year);
		dbManager.close();
		return ln;
	}
	...
	public Usuario crearUser(String s) {
		dbManager.open(false);
		Usuario a =dbManager.crearUser(s);
		dbManager.close();
