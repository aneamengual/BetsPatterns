...
public class DataAccess  {
	...
	public void initializeDB(){
		...
		try {
		  ...
		  // Estadistica est1= new Estadistica(year);
		   ...
		   Usuario u1= new Usuario("Kim", "Posible", "Posible", 54654564, "hjdcghdbc", "kimposible", fechaNacimiento, "63546738H", "llama", 51566565);
		   u1.addCupon();
		   Usuario u2= new Usuario("Manolo", "Gil", "Fernandez", 54654564, "hjdcghdbc", "manolito64", fechaNacimiento, "63546738H", "deOro", 51566565);
	       u2.addCupon();
	...
	public Vector<Event> getEvents(Date date) {
		...
		List<Event> events = extractedBDEvent(date);
	 	 for (Event ev:events){
	...
	public List<Event> extractedBDEvent(Date date) {
		TypedQuery<Event> query = db.createQuery("SELECT ev FROM Event ev WHERE ev.eventDate=?1 AND ev.finalizar=?2",Event.class);   
	...
	public ExtendedIterator<Event> getEventsIterator(Date date) {
		System.out.println(">> DataAccess: getEventsIterator");
		ExtendedIterator<Event> res = new ExtendedIteratorEvents();
		List<Event> events = extractedBDEvent(date);
		res.setIt(events);
		return res;
	}
	...
	public Vector<Event> getEventsEspecified(Date date, Seleccion sel) {
		System.out.println(">> DataAccess: getEvents");
		Vector<Event> res = new Vector<Event>();	
		TypedQuery<Event> query = db.createQuery("SELECT ev FROM Event ev WHERE ev.eventDate=?1 AND ev.finalizar=?2 AND ev.seleccion=?3",Event.class);   
		query.setParameter(1, date);
		query.setParameter(2, false);
	...
	public Vector<Date> getEventsMonth(Date date) {
		...
		TypedQuery<Date> query = db.createQuery("SELECT DISTINCT ev.eventDate FROM Event ev WHERE ev.eventDate BETWEEN ?1 and ?2 AND ev.finalizar=?3",Date.class);   
	...
	public Vector<Date> getEventsMonthEspecified(Date date, Seleccion sel) {
		...
		TypedQuery<Date> query = db.createQuery("SELECT DISTINCT ev.eventDate FROM Event ev WHERE ev.eventDate BETWEEN ?1 and ?2 AND ev.finalizar=?3 AND ev.seleccion=?4",Date.class);   
		query.setParameter(1, firstDayMonthDate);
		query.setParameter(2, lastDayMonthDate);
		query.setParameter(3, false);
...
public Usuario anadirUsuario(String nombre, String apellido1, String apellido2, int telefono, String correo,
		String nombreUsuario, Date fechaNacimiento, String DNI, String password, int numTarjeta)
	...
	if (us!=null || subd!=null) throw new UserAlreadyExists(ResourceBundle.getBundle("Etiquetas").getString("UserAlreadyExists"));
	
	db.getTransaction().begin();
	Usuario u= new Usuario(nombre, apellido1, apellido2, telefono, correo, nombreUsuario, fechaNacimiento, DNI, password, numTarjeta);
	u.addCupon();
	db.persist(u); 
	db.getTransaction().commit();
	return u;
...
public Usuario comprobar(String nombreUsuario, String password) throws WrongUserOrPassword {
	...
	if(user==null) throw new WrongUserOrPassword(ResourceBundle.getBundle("Etiquetas").getString("UserPassWordIsWrong"));
	else {
		String passW= user.getPassword();
		if(!passW.equals(password)) throw new WrongUserOrPassword(ResourceBundle.getBundle("Etiquetas").getString("UserPassWordIsWrong"));
	}
	return user;
...
public Apuesta anadirApuesta(Usuario user, Pronostico respuesta, AnadirApuestaParameter parameterObject, Event ev){
	System.out.println(">> DataAccess: CreateApuesta=> Usuario= "+ user+ " pronostico= "+respuesta + " apuesta= "+parameterObject.apuesta);
	...
	query.setParameter(1, parameterObject.year);
	List<Estadistica> estadistica=query.getResultList();
	Estadistica esta= estadistica.get(0);
	
	db.getTransaction().begin();
	Apuesta a= u.addApuesta(respuesta, ev, parameterObject.apuesta);
	e.addApuesta(user, respuesta, parameterObject.apuesta);
	sel.setCuentaDeApuestas(sel.getCuentaDeApuestas()+1);
	if(parameterObject.modoPago.equals("tarjeta")) {
		float dineroempresa=esta.getGanancias()+parameterObject.apuesta;
		esta.setGanancias(dineroempresa);
		
		float dinerouser= u.getMiMonedero()-parameterObject.apuesta;
		u.setMiMonedero(dinerouser);
	}else {
		float dineroCupon= cup.getDineroAcumulado()-parameterObject.apuesta;
...
public String anadirResultado(Question question, Pronostico pronostico) throws ResultAlreadyExists {
	...
	if(q.getResult()!=null)  throw new ResultAlreadyExists(ResourceBundle.getBundle("Etiquetas").getString("ResultAlreadyExists"));
...
public Usuario usuarioMasApostado() {
	...
	TypedQuery<Usuario> query = db.createQuery("SELECT u FROM Usuario u WHERE u.bloqueado=?1 ORDER BY u.cuentaDeApuestas DESC",Usuario.class); 
...
public Usuario usuarioMayorApuesta() {
	...
	int i=0; boolean encontrado=false; Usuario top=null;
...
public Vector<Seleccion> seleccionesMasApostadas() {
	...
	TypedQuery<Seleccion> query = db.createQuery("SELECT s FROM Seleccion s ORDER BY s.cuentaDeApuestas DESC",Seleccion.class); 
...
public ArrayList<Usuario> getListaNegra(int year){
	...
	db.getTransaction().begin();
	ArrayList<Usuario> listaN= esta.getListaNegra();
	for(Usuario u:listaN)System.out.println(u.getNombreUsuario());
	db.getTransaction().commit();
	return listaN;
}
	...
	public Mensaje guardarMensaje(Usuario user, String texto, Date date) {
		...
		Usuario u =db.find(Usuario.class, user.getNombreUsuario());
			...
			System.out.println(">> DataAccess: createMensaje=> emisor: " + u.getNombreUsuario() + " Texto=> " +texto);
				...
				//db.persist(q);
				db.persist(m); // db.persist(q) not required when CascadeType.PERSIST is added in questions property of Event class
								// @OneToMany(fetch=FetchType.EAGER, cascade=CascadeType.PERSIST)
				db.getTransaction().commit();
				return m;
	}
