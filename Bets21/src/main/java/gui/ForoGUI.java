...
public class ForoGUI extends JFrame {
	...
	private void Foro(domain.Usuario user) {
		...
		Vector<domain.Mensaje> mensajes =facade.getMensajesAll();
		...
		buton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(!label.getText().isBlank()) {
					Vector<domain.Mensaje> menta = facade.filtrarPorUsuario(label.getText());
					if(menta.isEmpty()) {
		...
		//IDEAS PARA AnADIR:
		//1. BOTON PARA RESPONDER A LAS PREGUNTAS Y QUE SE VEAN LAS RESPUESTAS DADAS EN ORDEN
		//2. Filtrar mensajes
	...
	private void enviarChat(Mensaje m) {
		...
		//CABECERA
		String cab= "From: " + m.getDueño().getNombreUsuario() + "             To: ALL "+ "     " + m.fechaBonita(m.getDate());
