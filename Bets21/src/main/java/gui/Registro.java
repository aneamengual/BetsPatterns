	...
	private void jButtonRegister_actionPerformed(ActionEvent e) {
		try {
			...
			if(nombre.length()!=0 && apellido1.length()!=0 && apellido2.length()!=0 && 
					correo.length()!=0 && dni.length()!=0 && nombreUsuario.length()!=0 && password.length()!=0) {
			
			BLFacade facade= MainGUI.getBusinessLogic();			
			Usuario user=facade.anadirUsuario(nombre, apellido1, apellido2, telefono, correo, nombreUsuario, fechaNacimiento, dni, password, numTarjeta);	
