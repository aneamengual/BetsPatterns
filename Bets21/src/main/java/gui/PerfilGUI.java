...
public class PerfilGUI extends JFrame {
	...
	public PerfilGUI(domain.Usuario user){
		try{
			Perfil(user);
		}
		catch(Exception e){
	 ...
	 * Create the frame.
	 */
	private void Perfil(Usuario user) {
		this.user=user;
		...
		jLabelNombre.setText(user.getNombre());
		...
		jLabelApellido1.setText(user.getApellido1());
		
		contentPane.add(jLabelApellido1);
		jLabelApellido2.setBounds(185, 101, 116, 13);
		jLabelApellido2.setText(user.getApellido2());
		...
		jLabelTelefono.setText(String.valueOf(user.getTelefono()));
