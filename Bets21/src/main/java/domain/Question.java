...
public class Question implements Serializable {
	...
	public Pronostico addPronostico(String pronostico, float ganancia)  {
        Pronostico p=new Pronostico(this, pronostico,ganancia);
        pronosticos.add(p);
        return p;
	}
	...
	public boolean DoesPronosticoExists(String pronostico)  {	
		for (Pronostico p:this.getPronosticos()){
			if (p.getSolucion().compareTo(pronostico)==0)
				return true;
		}
		return false;
	}
	
	public boolean tengoRestultado() {
